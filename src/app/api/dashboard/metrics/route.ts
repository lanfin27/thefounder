...
async function getOverviewMetrics() {
  ...
  const completionRate = totalListings && totalListings > 0
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      highlights: {
        ...
        growthRate: recentListings && recentListings > 0 ? '+' + recentListings : '0',
        topMetric: totalListings && totalListings > 5000 ? 'Large Dataset' : 'Growing Dataset'
      }
    }
  })
}
  ...
  return NextResponse.json({
    ...
    data: {
      realtime: {
        ...
        status: (hourCount.count || 0) > 0 ? 'Active' : 'Idle',
