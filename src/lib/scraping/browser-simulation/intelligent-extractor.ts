...
export class IntelligentDataExtractor {
  ...
  private async extractSelective(
    ...
    for (const element of textElements) {
      ...
      for (const [interest, weight] of interestMap) {
  ...
  private async shouldStopReading(result: ExtractedData): boolean {
  ...
  getExtractionSummary(): Record<string, any> {
    ...
    for (const extraction of this.extractedData.values()) {
      ...
      // Analyze focus areas
      for (const [element, time] of Object.entries(extraction.metadata.focusTime)) {
        summary.readingBehavior.focusAreaCoverage[element] = 
          (summary.readingBehavior.focusAreaCoverage[element] || 0) + time;
      }
    }

    summary.averageConfidence = totalConfidence / this.extractedData.size;
    summary.averageConfidence = totalReadingTime / this.extractedData.size;
