...
export function LatestPosts({ posts }: LatestPostsProps) {
  console.log('🔵 LatestPosts - 인라인 스타일 강제 적용 버전 2025-11-17!');
                ...
                <div className="block md:hidden">
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      {/* 🔴 제목 - 인라인 스타일 강제 적용 */}
                      <h3
                        className="text-base font-semibold leading-tight text-gray-900 line-clamp-1 group-hover:text-blue-600 transition-colors"
                        style={{
                          marginBottom: '8px',
                          display: '-webkit-box',
                          WebkitLineClamp: 1,
...
console.log('\n🔍 검증:');
console.log('  인라인 스타일 버전:', content.includes('인라인 스타일 강제 적용 버전') ? '✅' : '❌');
console.log('  marginBottom 8px:', content.includes("marginBottom: '8px'") ? '✅' : '❌');
console.log('  marginTop 12px:', content.includes("marginTop: '12px'") ? '✅' : '❌');
console.log('  WebkitLineClamp:', content.includes('WebkitLineClamp') ? '✅' : '❌');

// 코드 라인 확인
const lines = content.split('\n');
console.log('\n📄 중요 코드 라인:');
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('marginBottom')) {
    console.log(`  Line ${i + 1}: ${lines[i].trim()}`);
  }
  if (lines[i].includes('marginTop')) {
    console.log(`  Line ${i + 1}: ${lines[i].trim()}`);
  }
}
