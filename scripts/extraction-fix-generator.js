// scripts/extraction-fix-generator.js
const fs = require('fs');
const path = require('path');

class ExtractionFixGenerator {
  constructor() {
    this.diagnosticReport = null;
    this.fixedExtractor = null;
  }

  async generateFixes() {
    console.log('🔧 EXTRACTION FIX GENERATOR');
    console.log('=' .repeat(60));
    
    // Load the most recent diagnostic report
    const reportFiles = fs.readdirSync('.')
      .filter(f => f.startsWith('extraction-diagnostic-report-'))
      .sort((a, b) => b.localeCompare(a));
    
    if (reportFiles.length === 0) {
      console.error('❌ No diagnostic report found. Run extraction-diagnostic-system.js first.');
      return;
    }
    
    const latestReport = reportFiles[0];
    console.log(`📋 Loading diagnostic report: ${latestReport}`);
    
    this.diagnosticReport = JSON.parse(fs.readFileSync(latestReport, 'utf8'));
    
    console.log('\n📊 Diagnostic Summary:');
    console.log(`   Critical issues: ${this.diagnosticReport.criticalFindings?.length || 0}`);
    console.log(`   Recommendations: ${this.diagnosticReport.recommendations?.length || 0}`);
    console.log(`   Current extraction rates:`);
    if (this.diagnosticReport.extractionRates) {
      console.log(`     - Title: ${this.diagnosticReport.extractionRates.title}%`);
      console.log(`     - Revenue: ${this.diagnosticReport.extractionRates.revenue}%`);
      console.log(`     - Multiple: ${this.diagnosticReport.extractionRates.multiple}%`);
    }
    
    // Generate the fixed extractor
    this.generateFixedExtractor();
    
    // Save the fixed extractor
    this.saveFixedExtractor();
    
    console.log('\n✅ Fix generation complete!');
    console.log('🚀 Next step: Run improved-working-extractor.js to see dramatic improvements');
  }

  generateFixedExtractor() {
    console.log('\n🛠️ Generating Fixed Extractor...');
    
    // Build optimized selectors based on diagnostic findings
    const optimizedSelectors = this.buildOptimizedSelectors();
    
    // Generate the complete fixed extractor code
    this.fixedExtractor = `// scripts/improved-working-extractor.js
...
class ImprovedWorkingExtractor {
  constructor() {
    ...
    this.selectors = ${JSON.stringify(optimizedSelectors, null, 6)};
  }
  ...
  async extractPage(browser, pageNum) {
    ...
    try {
      ...
      await page.waitForSelector('${this.getContainerSelector()}', { 
        timeout: 10000 
      }).catch(() => {
      ...
      const listings = await page.evaluate((selectors) => {
        ...
        ${this.getContainerDetectionCode()}
...
new ImprovedWorkingExtractor().execute().catch(console.error);`;
  ...
  buildOptimizedSelectors() {
    ...
    // Use discovered working selectors from diagnostic
    if (this.diagnosticReport.workingSelectors) {
      Object.entries(this.diagnosticReport.workingSelectors).forEach(([field, selectors]) => {
        // Add high-confidence selectors first
        const highConfidence = selectors.filter(s => s.confidence === 'high');
        const mediumConfidence = selectors.filter(s => s.confidence === 'medium');
        
        highConfidence.forEach(s => {
          if (!optimized[field].includes(s.selector)) {
            optimized[field].push(s.selector);
          }
        });
    ...
    const fallbacks = {
      title: ['h3', 'h2', '.listing-title', '[class*="title"]'],
      price: ['[class*="price"]', 'span:contains("$")', 'div:contains("$")'],
      revenue: ['[class*="revenue"]', '[class*="profit"]', 'span:contains("/mo")'],
      multiple: ['[class*="multiple"]', 'span:contains("x")', '[class*="mult"]'],
      url: ['a[href^="/"]', 'a[href*="listing"]'],
      category: ['[class*="category"]', '[class*="type"]', '.industry']
    };
    
    Object.entries(fallbacks).forEach(([field, selectors]) => {
      selectors.forEach(selector => {
        if (optimized[field].length < 5 && !optimized[field].includes(selector)) {
          optimized[field].push(selector);
        }
      });
    });
  ...
  getContainerSelector() {
    // Use the best container selector from diagnostic
    if (this.diagnosticReport.domStructure?.listingContainers?.length > 0) {
      return this.diagnosticReport.domStructure.listingContainers[0].selector;
    }
    
    // Fallback to common patterns
    return '[class*="listing"], [class*="card"], [class*="item"]';
  }

  getContainerDetectionCode() {
    // Use container-based detection if available
    if (this.diagnosticReport.domStructure?.listingContainers?.length > 0) {
      const selector = this.diagnosticReport.domStructure.listingContainers[0].selector;
      return `const containers = Array.from(document.querySelectorAll('${selector}'));`;
    }
    
    // Fallback to improved price-based detection
    return `
  ...
  saveFixedExtractor() {
    const filename = 'improved-working-extractor.js';
    fs.writeFileSync(path.join('scripts', filename), this.fixedExtractor);
    
    console.log(`\n✅ Fixed extractor saved: scripts/${filename}`);
    
    // Also save a summary of fixes applied
    const fixSummary = {
      timestamp: new Date().toISOString(),
      diagnosticReport: this.diagnosticReport.summary,
      appliedFixes: [
    ...
    fs.writeFileSync('extraction-fixes-applied.json', JSON.stringify(fixSummary, null, 2));
    console.log('📋 Fix summary saved: extraction-fixes-applied.json');
  }
}

// Execute fix generation
async function generateFixes() {
  const generator = new ExtractionFixGenerator();
  await generator.generateFixes();
}

generateFixes().catch(console.error);